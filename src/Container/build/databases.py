    ...
    for databases in self.databaseContainer:
        ...
        try:
            databaseContainerID = DockerClient.containers.create(
                ...
                hostname=f"{self.gitRepo[2]}_{databases}",  # 데이터베이스 컨테이너 호스트 이름 파라미터
                name=f"{self.gitRepo[2]}_{databases}",  # 도커 컨테이너 이름 파라미터
